...
int main(int argc, char **argv){
	...
	initiate_threads(3);
	printf("imprimiendo threads");
	start_threads(3, port);
	//job();
	while(1){
...
int initiate_threads(int amount){
...
int start_threads(int amount, int port){
	for(int i = 0;i<amount;i++){
    	pthread_create(&threads[i], NULL, job,NULL);
	}
}